...
//    Pan - middle mouse, or arrow keys / touch: three finter swipe

THREE.OrbitControls = function ( object, scene, domElement ) {
	...
	this.myZoom = function ( vec3dir )
	{
		...
		//scope.target = position.clone().sub((position.clone().sub(scope.target)).normalize());
	};
	...
	function onMouseWheel( event ) {
		...
		var scalar = 250;//минимальный шаг если под курсором нет ничего
		var intersects = raycaster.intersectObjects(element.scene.children, true);
		if(intersects.length > 0)
			scalar = Math.pow(intersects[0].distance, 0.7);	//проверка не "бесконечный zoom" не нужна, т.к. есть zNear камеры, в raycaster.setFromCamera
